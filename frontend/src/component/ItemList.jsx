...
const ItemList = () => {
  ...
  const handleAccept = async (tradeId) => {
    try {
      const response = await fetch("http://localhost:2000/trade/update-status", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "auth-token": localStorage.getItem("auth-token"),
      ...
      if (data.success) {
        ...
        setItems((prevItems) =>
          prevItems.filter((item) => item.tradeId !== tradeId)
        );
      } else {
        setErrorMessage(data.error);
        setSuccessMessage("");
            ...
            {items.map((item) => (
              ...
              >
                ...
                <div className="flex space-x-4">
                  {/* Accept (Tick) Button */}
                  <button
                    onClick={() => handleAccept(item.tradeId)} // Pass trade ID
